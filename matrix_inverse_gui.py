...
class MatrixInverseApp:
    ...
    def _build_layout(self) -> None:
        ...
        self.clear_btn = ttk.Button(controls, text="Clear", command=self.clear_results)
        ...
        results_section.columnconfigure(1, weight=0)

        results_label = ttk.Label(results_section, text="Inverse (Result)")
        results_label.grid(row=0, column=0, sticky="w", pady=(0, 6))

        # Results Text widget with vertical scrollbar (read-only)
        self.results_vscroll = ttk.Scrollbar(results_section, orient="vertical")
        self.results_vscroll.grid(row=1, column=1, sticky="ns")
        ...
        self.results_vscroll.configure(command=self.results_text.yview)
    ...
    # Public API for upcoming compute action to render inverse matrix
    def render_inverse_result(self, inv: list[list[float]]) -> None:
        """
        Render the inverse matrix into the results Text widget with exactly
        4-decimal formatting for display only (no mutation of input).

        Args:
            inv: A list of rows (each a list of floats) with consistent lengths.
        """
        # Build textual representation with tabs to align columns in monospace
        lines: list[str] = []
        for row in inv:
            formatted = ["{:.4f}".format(val) for val in row]
    ...
    def clear_results(self) -> None:
        """Clear the results Text widget safely (toggle state during update)."""
        self.results_text.configure(state="normal")
        self.results_text.delete("1.0", tk.END)
        self.results_text.configure(state="disabled")
